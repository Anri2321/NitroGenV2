...
import time
LICNECE = """
Hi Thanks For Using AirDrop NitroGenerator made by Anis X Urgot X ilws X Tsuyu : Discalmer : if you need help contact me "Anis#1708" (My discord)
"""
...
try:
    ...
    # Tell the user
    input("Please Check If you are connecting to the internet and try again.\nPress enter to exit")
    exit()  # Exit program
    ...
    def main(self):  # The main function contains the most important code
        ...
        if os.name == "nt":  # If the system is windows
            ...
            ctypes.windll.kernel32.SetConsoleTitleW(
                "Nitro Generator and Checker - Made by Anis")  # Change the
        else:  # Or if it is unix
            print(f'\33]0;Nitro Generator and Checker - Made by Anis X ilws X Urgot X Tsuyu\a',
...
╚═╝░░╚═╝╚═╝╚═╝░░╚═╝╚═════╝░╚═╝░░╚═╝░╚════╝░╚═╝░░░░░
        ...
        self.slowType("Made by: Anis X ilws X Urgot X Tsuyu", .02)
        ...
        if response.status_code == 200:  # If the responce went through
            ...
            if notify is not None:  # If a webhook has been added
                DiscordWebhook(  # Send the message to discord letting the user know there has been a valid nitro code
                    url=url,
                    content=f"UN NITRO VIENT D'APPARAITRE ! @everyone \n{nitro}"
                ).execute()
